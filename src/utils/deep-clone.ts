 ...
 */
export const deepClone = (src: any, clonesMap = new Map()): any => {
  // primitives & functions
  if (!src || typeof src !== 'object' || typeof src === 'function') {
    return src;
  }

  if (typeof window !== 'undefined' && src === window) {
    return src;
  }
  ...
  if (src instanceof Error) {
    const newError = new (src.constructor as ErrorConstructor)(src.message);
  ...
  if (src instanceof Map) {
    ...
    clonedEntries.forEach((entry: [any, any]) => clone.set(...entry));
    return clone;
  }
  ...
  if (src instanceof Set) {
    ...
    clonedValues.forEach((entry: any) => clone.add(entry));
    return clone;
  }
