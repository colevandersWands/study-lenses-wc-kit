...
describe('study load', () => {
	...
	it('should add default config for lenses without config', () => {
		...
		expect(typeof (lensRegistry as any)['minimal-lens'].config).toBe(
			'function'
		);
	...
	it('should preserve existing lens register function if provided', () => {
		const mockRegister = () => 'sl-custom-lens';
		const lensWithRegister = {
			name: 'register-lens',
			lens: mockLens,
			register: mockRegister,
		};

		const result = load(lensWithRegister);

		expect(result).toBe(true);
		expect((lensRegistry as any)['register-lens'].register).toBe(
			mockRegister
		);
	});
