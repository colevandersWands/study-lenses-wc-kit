/**
 * Loop Guard AST Transformation - Exact copy from insert-loop-guards.ts
 * 
 ...
 * 2. Walking the AST to find all loop constructs (for, while, do-while, for-in, for-of, for-await-of)
...
/**
 * Maps AST node types to configuration loop types
 */
const getLoopType = (node: Node): LoopType | null => {
  switch (node.type) {
    case 'ForStatement':
      return 'for';
    case 'WhileStatement':
    ...
    case 'ForInStatement':
      return 'for-in';
    case 'ForOfStatement':
      // Distinguish for-await-of from for-of
      return (node as any).await === true ? 'for-await-of' : 'for-of';
    default:
      return null;
  }
};
...
// =============================================================================
// Main Loop Guard Insertion Function (Enhanced for Configuration)
// =============================================================================

/**
 * Inserts loop guards into JavaScript code to prevent infinite loops
 * Enhanced version that supports configuration-based loop type filtering
 * 
 * This is the main entry point for the loop guard system. It accepts either
 * raw JavaScript code as a string or a pre-parsed AST, processes configured loop
 ...
 * - for-await-of loops (async for-of)
 * 
 * @param evalCode - Input JavaScript code as string or parsed AST
 * @param config - Configuration object specifying which loops to guard and limits
 * @returns Modified code/AST with loop guards inserted, or original if no configured loops found
 * 
 ...
 * `;
 * 
 * const config = { max: 100, loops: ['while'], format: false };
 * const safeCode = insertLoopGuards(unsafeCode, config);
 * // Result includes automatic loop counter and safety check
 * ```
 */
export const insertLoopGuards = (evalCode: CodeInput, config: LoopGuardConfig): CodeOutput => {
  ...
  // Walk the AST and transform configured loop constructs
  const guardedTree = (walk as any)(ast, {
    ...
    leave(node: any, parent: any, _prop: string | number, _index: number) {
      // Get the loop type for this node
      const loopType = getLoopType(node);
      
      // Only process if this is a configured loop type
      if (!loopType || !config.loops.includes(loopType)) {
        return;
      }
      ...
      const { variable, check } = generateLoopGuard(loopNumber, config.max);
      
      // Ensure loop body is a block statement for safe insertion
      if ((node as any).body && (node as any).body.type !== 'BlockStatement') {
  ...
  const guarded: CodeOutput = typeof evalCode === 'object' ? guardedTree as Node : recast.print(guardedTree as any).code;
...
// =============================================================================
// Original Utility Functions (for compatibility)
// =============================================================================

/**
 * Original insertLoopGuards function that processes all loop types
 * Maintained for backward compatibility
 */
export const insertLoopGuardsAll = (evalCode: CodeInput, maxIterations: number): CodeOutput => {
  // Parse input if it's a string, otherwise use the provided AST
  const ast = typeof evalCode === 'object' ? evalCode : recast.parse(evalCode);
  ...
  const guardedTree = (walk as any)(ast, {
    ...
    leave(node: any, parent: any, _prop: string | number, _index: number) {
      ...
      // Generate unique loop guard for this loop
      const { variable, check } = generateLoopGuard(loopNumber, maxIterations);
