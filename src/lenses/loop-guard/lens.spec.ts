...
describe('loop-guard lens', () => {
  ...
  describe('for loop protection', () => {
    ...
    it('should handle nested for loops with unique guards', async () => {
      ...
      expect(result.snippet.code).toContain('loopGuard_1');
      expect(result.snippet.code).toContain('loopGuard_2');
      expect(result.snippet.code).not.toContain('loopGuard_3');
    });
  });
  ...
  describe('for-await-of loop protection', () => {
    it('should add loop guards to for-await-of loop', async () => {
      ...
      if (result.snippet.code === snippet.code) {
        // Parser couldn't handle for-await-of, which is acceptable
        expect(result.snippet.code).toBe(snippet.code);
      } else {
  ...
  describe('configuration handling', () => {
    it('should respect custom max iteration limit', async () => {
      ...
      expect(result.snippet.code).toContain('if (loopGuard_1 > 500)');
      expect(result.snippet.code).toContain('loopGuard_1 is greater than 500');
    });
    ...
    it('should only guard specified loop types', async () => {
      ...
      expect(result.snippet.code).not.toContain('loopGuard_2');
    });

    it('should respect format config setting', async () => {
      const snippet = createSnippet(`for(let i=0;i<10;i++){console.log(i);}`);
      ...
      expect(result.snippet.code).toContain('for '); // recast normalizes spacing
    });
    ...
    it('should use default config when none provided', async () => {
      ...
      expect(result.snippet.code).toContain('if (loopGuard_1 > 1000)'); // Default max
    });
  });
  ...
  describe('mixed loop types', () => {
    it('should handle multiple different loop types', async () => {
      ...
      expect(result.snippet.code).toContain('loopGuard_1');
      expect(result.snippet.code).toContain('loopGuard_2');
      expect(result.snippet.code).toContain('loopGuard_3');
      expect(result.snippet.code).not.toContain('loopGuard_4');
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle code without loops', async () => {
      ...
      expect(result.snippet.code).not.toContain('loopGuard');
    ...
    it('should preserve snippet metadata', async () => {
      ...
      expect(result.snippet.lang).toBe('typescript');
  ...
  describe('already guarded code', () => {
    it('should add new guards even to code with existing guards', async () => {
      ...
      const guardCount = (result.snippet.code.match(/loopGuard_/g) || []).length;
  ...
  describe('return value structure', () => {
    ...
    it('should return modified snippet with loop guards', async () => {
      ...
      expect(result.snippet.code).not.toEqual(snippet.code);
      expect(result.snippet.code).toContain('loopGuard_1');
      expect(result.snippet.lang).toBe(snippet.lang);
      expect(result.snippet.test).toBe(snippet.test);
    });
  });
});